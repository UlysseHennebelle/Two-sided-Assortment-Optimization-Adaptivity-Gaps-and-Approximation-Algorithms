"""Run the corrected Section 7 adaptive-policy comparison."""
...
def main() -> None:
    ...
    allowed_sizes = set(config["adaptivity_sizes"])
    source = (
        item
    ...
    for generated in bounded_instances(source, args.max_size, args.max_instances):
        part_id = f"{generated.instance_id}-section7-adaptivity"
        if dataset_part_path(args.output_root, "algorithm_runs", part_id).exists() and not args.no_resume:
            ...
            continue
        runs, replications = run_section7_adaptivity(generated, config, digest)
